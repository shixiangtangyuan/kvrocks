...
rocksdb::Status List::push(const Slice &user_key, const std::vector<Slice> &elems, bool create_if_missing, bool left,
  ...
  *new_size = 0;
  ...
  auto cmd = left ? RedisListCommand::kCmdLPush : RedisListCommand::kCmdRPush;
  WriteBatchLogData log_data(kRedisList, {EnumToString(cmd)});
...
rocksdb::Status List::PopMulti(const rocksdb::Slice &user_key, bool left, uint32_t count,
  ...
  auto batch = storage_->GetWriteBatchBase();
  auto cmd = left ? RedisListCommand::kCmdLPop : RedisListCommand::kCmdRPop;
  WriteBatchLogData log_data(kRedisList, {EnumToString(cmd)});
  batch->PutLogData(log_data.Encode());
...
rocksdb::Status List::Rem(const Slice &user_key, int count, const Slice &elem, uint64_t *removed_cnt,
                          int64_t *operate_count) {
  *removed_cnt = 0;
  int64_t op_count = 0;
  if (!operate_count) operate_count = &op_count;
  *operate_count = 0;
  ...
  *operate_count = 1;
  for (iter->Seek(start_key); iter->Valid() && iter->key().starts_with(prefix);
       !reversed ? (iter->Next(), *operate_count += 1) : (iter->Prev(), *operate_count += 1)) {
    if (iter->value() == elem) {
  ...
  WriteBatchLogData log_data(kRedisList,
                             {EnumToString(RedisListCommand::kCmdLRem), std::to_string(count), elem.ToString()});
  batch->PutLogData(log_data.Encode());
...
rocksdb::Status List::Insert(const Slice &user_key, const Slice &pivot, const Slice &elem, bool before, int *new_size,
  ...
  *operate_count = 0;
  ...
  auto iter = util::UniqueIterator(storage_, read_options);
  *operate_count = 1;
  for (iter->Seek(start_key); iter->Valid() && iter->key().starts_with(prefix); iter->Next(), *operate_count += 1) {
    if (iter->value() == pivot) {
      InternalKey ikey(iter->key(), storage_->IsSlotIdEncoded());
      Slice sub_key = ikey.GetSubKey();
  ...
  WriteBatchLogData log_data(
      kRedisList, {EnumToString(RedisListCommand::kCmdLInsert), before ? "1" : "0", pivot.ToString(), elem.ToString()});
  batch->PutLogData(log_data.Encode());
  ...
  for (; iter->Valid() && iter->key().starts_with(prefix);
       !reversed ? (iter->Next(), *operate_count += 1) : (iter->Prev(), *operate_count += 1)) {
    buf.clear();
...
rocksdb::Status List::Set(const Slice &user_key, int index, Slice elem) {
  ...
  auto batch = storage_->GetWriteBatchBase();
  WriteBatchLogData log_data(kRedisList, {EnumToString(RedisListCommand::kCmdLSet), std::to_string(index)});
  batch->PutLogData(log_data.Encode());
  batch->Put(sub_key, elem);
  return storage_->Write(storage_->DefaultWriteOptions(), batch->GetWriteBatch());
}
...
rocksdb::Status List::lmoveOnSingleList(const rocksdb::Slice &src, bool src_left, bool dst_left, std::string *elem) {
  ...
  auto batch = storage_->GetWriteBatchBase();
  WriteBatchLogData log_data(kRedisList, {EnumToString(RedisListCommand::kCmdLMove), src.ToString(), src.ToString(),
...
rocksdb::Status List::lmoveOnTwoLists(const rocksdb::Slice &src, const rocksdb::Slice &dst, bool src_left,
  ...
  auto batch = storage_->GetWriteBatchBase();
  WriteBatchLogData log_data(kRedisList, {EnumToString(RedisListCommand::kCmdLMove), src.ToString(), dst.ToString(),
                                          src_left ? "left" : "right", dst_left ? "left" : "right"});
  batch->PutLogData(log_data.Encode());
...
rocksdb::Status List::Trim(const Slice &user_key, int start, int stop, int64_t *operate_count) {
  uint32_t trim_cnt = 0;
  int64_t op_count = 0;
  if (!operate_count) {
    operate_count = &op_count;
  }
  *operate_count = 0;
  std::string ns_key = AppendNamespacePrefix(user_key);
  ...
  auto batch = storage_->GetWriteBatchBase();
  WriteBatchLogData log_data(kRedisList, std::vector<std::string>{EnumToString(RedisListCommand::kCmdLTrim),
  ...
  metadata.Encode(&bytes);
  batch->Put(metadata_cf_handle_, ns_key, bytes);
  *operate_count = static_cast<int64_t>(trim_cnt);
  return storage_->Write(storage_->DefaultWriteOptions(), batch->GetWriteBatch());
}
}  // namespace redis
