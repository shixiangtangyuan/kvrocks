...
rocksdb::Status String::updateRawValue(const std::string &ns_key, const std::string &raw_value) {
  ...
  WriteBatchLogData log_data(kRedisString, {EnumToString(RedisStringCommand::kCmdSet)});
...
rocksdb::Status String::IncrByFloat(const std::string &user_key, double increment, double *new_value) {
  ...
  if (!value.empty()) {
    ...
    if (!n_stat || isspace(value[0])) {
      return rocksdb::Status::InvalidArgument(errValueIsNotFloat);
    }
    n = *n_stat;
  }
...
rocksdb::Status String::MSet(const std::vector<StringPair> &pairs, uint64_t ttl, bool lock) {
  ...
  WriteBatchLogData log_data(kRedisString);
  if (ttl > 0) {
    log_data.GetCmdLogDataObj().SetCmdArgs({EnumToString(RedisStringCommand::kCmdSetEX)});
  } else if (pairs.size() > 1) {
    log_data.GetCmdLogDataObj().SetCmdArgs({EnumToString(RedisStringCommand::kCmdMSet)});
  } else {
    log_data.GetCmdLogDataObj().SetCmdArgs({EnumToString(RedisStringCommand::kCmdSet)});
  }
  batch->PutLogData(log_data.Encode());
