...
class ZSet : public SubKeyScanner {
 public:
  ...
  rocksdb::Status Scan(const Slice &user_key, std::string *cursor, uint64_t limit, const std::string &member_prefix,
                       std::vector<std::string> *members, std::vector<double> *scores = nullptr,
  ...
  rocksdb::Status RangeByLex(const Slice &user_key, const RangeLexSpec &spec, MemberScores *mscores,
                             uint64_t *removed_cnt, int64_t *seek_count = nullptr);
