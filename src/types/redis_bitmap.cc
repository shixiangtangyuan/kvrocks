...
rocksdb::Status Bitmap::SetBit(const Slice &user_key, uint32_t offset, bool new_bit, bool *old_bit) {
  ...
  *old_bit = (value[byte_index] & (1 << bit_offset)) != 0;
  ...
  WriteBatchLogData log_data(kRedisBitmap, {EnumToString(RedisBitmapCommand::kCmdSetBit), std::to_string(offset)});
...
rocksdb::Status Bitmap::BitOp(BitOpFlags op_flag, const std::string &op_name, const Slice &user_key,
  ...
  std::vector<std::string> log_args = {EnumToString(RedisBitmapCommand::kCmdBitOp), op_name};
  for (const auto &op_key : op_keys) {
...
bool Bitmap::bitfieldWriteAheadLog(const ObserverOrUniquePtr<rocksdb::WriteBatchBase> &batch,
                                   const std::vector<BitfieldOperation> &ops) {
  std::vector<std::string> cmd_args{EnumToString(RedisBitmapCommand::kCmdBitfield)};
