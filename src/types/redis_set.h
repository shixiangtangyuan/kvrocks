...
class Set : public SubKeyScanner {
 public:
  ...
  rocksdb::Status Members(const Slice &user_key, std::vector<std::string> *members, uint64_t *seek_count = nullptr);
  ...
  rocksdb::Status Scan(const Slice &user_key, std::string *cursor, uint64_t limit, const std::string &member_prefix,
                       std::vector<std::string> *members, int64_t *seek_count = nullptr);
