...
bool MetadataFilter::Filter(int level, const Slice &key, const Slice &value, std::string *new_value,
                            bool *modified) const {
  if (!stor_->GetConfig()->compact_with_filter_flag.load()) return false;
  if (!stor_->GetConfig()->ShouldMetadataFilterAtLevel(level)) return false;
  ...
  DLOG(INFO) << "[compact_filter/metadata] "
             ...
             << ", result: " << (metadata.Expired() ? "deleted" : "reserved") << ", level: " << level;
...
rocksdb::CompactionFilter::Decision SubKeyFilter::FilterBlobByKey(int level, const Slice &key, std::string *new_value,
  ...
  if (!stor_->GetConfig()->compact_with_filter_flag.load()) return rocksdb::CompactionFilter::Decision::kKeep;
  if (!stor_->GetConfig()->ShouldSubkeyFilterAtLevel(level)) return rocksdb::CompactionFilter::Decision::kKeep;
...
bool SubKeyFilter::Filter(int level, const Slice &key, const Slice &value, std::string *new_value,
                          bool *modified) const {
  if (!stor_->GetConfig()->compact_with_filter_flag.load()) return false;
  if (!stor_->GetConfig()->ShouldSubkeyFilterAtLevel(level)) return false;
  ...
  if (metadata.Type() == kRedisHash) {
    if (metadata.HasSubFlag()) {
      ...
      rocksdb::Status rs = sub_data.Decode(&metadata, value);
      if (!rs.ok()) {
        if (rs.IsNotFound()) {
          return true;
        }
        LOG(ERROR) << "[compact_filter/subkey] Failed to decode sub data"
                   << ", namespace: " << ikey.GetNamespace() << ", key: " << ikey.GetKey()
                   << ", err: " << rs.ToString();
        return false;
      }
    }
