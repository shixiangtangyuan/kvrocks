...
class Database {
 public:
  ...
  [[nodiscard]] rocksdb::Status Scan(const std::string &cursor, int16_t *slot, int16_t end_slot, uint64_t count,
                                     const std::string &pattern, RedisType type, std::vector<std::string> *keys,
                                     std::string *end_cursor = nullptr, bool for_copi2 = false,
...
class SubKeyScanner : public redis::Database {
 public:
  ...
  rocksdb::Status Scan(RedisType type, const Slice &user_key, std::string *cursor, uint64_t limit,
                       const std::string &subkey_prefix, std::vector<std::string> *keys,
                       std::vector<std::string> *values = nullptr, int64_t *seek_count = nullptr);
};
...
class WriteBatchLogData {
 public:
  ...
  WriteBatchLogData(RedisType type, std::vector<std::string> &&args) : type_(type), cmd_log_data_(std::move(args)) {}

  RedisType GetRedisType() const { return type_; }
  CmdLogData &GetCmdLogDataObj() { return cmd_log_data_; }
  CDCLogData &GetCDCLogDataObj() { return cdc_log_data_; }
  void SetEnableCDCSync() { enbale_cdc_sync_ = true; }
 ...
 private:
  ...
  CmdLogData cmd_log_data_;
  CDCLogData cdc_log_data_;

  bool enbale_cdc_sync_ = false;
};
