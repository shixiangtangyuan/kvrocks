...
StatusOr<ParseResultAndPos<T>> TryParseInt(const char *v, int base = 0) {
  ...
  if (v == end) {
    return {Status::NotOK, "value is not an integer or out of range"};
  }
...
StatusOr<T> SimpleAtoi(const char *data, size_t size) {
  T v{0};
  absl::string_view s(data, size);
  if (absl::SimpleAtoi(s, &v)) return v;
  return {Status::NotOK, "value not a integer"};
}

inline StatusOr<double> SimpleAtod(const char *data, size_t size) {
  double v{0};
  absl::string_view s(data, size);
  if (absl::SimpleAtod(s, &v)) return v;
  return {Status::NotOK, "value not a float"};
}
